...
#[test]
fn sources_template() {
    let mut template = SourcesPart::blank_template();
    assert_eq!(
...
fn all_crates_template() {
    let mut template = AllCratesPart::blank_template();
...
fn search_index_template() {
    let mut template = SearchIndexPart::blank_template();
    assert_eq!(
        but_last_line(&template.to_string()),
...
fn trait_alias_template() {
    let mut template = TraitAliasPart::blank_template();
    assert_eq!(
        but_last_line(&template.to_string()),
        r#"(function() {
...
fn type_alias_template() {
    let mut template = TypeAliasPart::blank_template();
    assert_eq!(
        but_last_line(&template.to_string()),
        r#"(function() {
