...
pub(crate) fn write_shared(
    ...
    if opt.emit.is_empty() || opt.emit.contains(&EmitType::InvocationSpecific) {
        if cx.include_sources {
            write_rendered_cci::<SourcesPart, _>(SourcesPart::blank_template, dst, &crates)?;
        }
        write_rendered_cci::<SearchIndexPart, _>(SearchIndexPart::blank_template, dst, &crates)?;
        write_rendered_cci::<AllCratesPart, _>(AllCratesPart::blank_template, dst, &crates)?;
    }
    write_rendered_cci::<TraitAliasPart, _>(TraitAliasPart::blank_template, dst, &crates)?;
    write_rendered_cci::<TypeAliasPart, _>(TypeAliasPart::blank_template, dst, &crates)?;
